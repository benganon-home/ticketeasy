...
import { MapPin, Clock, Shield, Star, Tag, Users, AlertTriangle, MessageCircle, Heart } from 'lucide-react';
import { formatPrice, formatDate, formatTime, getHotnessLabel } from '../../data/mockData';
import { getEvent } from '../../services/events';
import { getListingsForEvent } from '../../services/listings';
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            {offer.originalPrice && offer.price > offer.originalPrice && (
              <span className="text-[10px] text-dark-300">+{(((offer.price / offer.originalPrice) - 1) * 100).toFixed(0)}%</span>
            )}
            {offer.originalPrice && offer.price <= offer.originalPrice && (
      ...
      <div className="flex items-center justify-between pt-2 border-t border-dark-100 dark:border-dark-600">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-full bg-primary-100 dark:bg-primary-900/30 flex items-center justify-center">
            {offer.sellerPhotoURL
              ? <img src={offer.sellerPhotoURL} alt="" className="w-full h-full rounded-full object-cover" />
              : <span className="text-sm font-600 text-primary-600 dark:text-primary-300">{offer.sellerName?.charAt(0)}</span>
            }
          </div>
          <div>
            <p className="text-xs font-600">{offer.sellerName}</p>
            {offer.sellerRating && (
              <div className="flex items-center gap-1">
                <Star className="w-3 h-3 text-amber-400 fill-amber-400" />
                <span className="text-[10px] text-dark-400">{offer.sellerRating}</span>
              </div>
            )}
          </div>
        </div>
...
function MakeOfferModal({ event, onClose }) {
  ...
  const maxPrice = Math.round((event.originalPrice || event.minPrice || 0) * 1.2);
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-lg bg-white dark:bg-dark-800 rounded-t-3xl sm:rounded-3xl p-6 animate-slide-up" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="text-sm text-dark-300 dark:text-dark-400 mb-4">
          מחיר מקורי: {formatPrice(event.originalPrice || event.minPrice)} · מקסימום: {formatPrice(maxPrice)}
        </p>
        <div className="relative mb-4">
          <span className="absolute right-4 top-1/2 -translate-y-1/2 text-dark-300 font-600">₪</span>
          <input type="number" value={price} onChange={(e) => setPrice(e.target.value)} placeholder="הזן סכום" className="input-field pr-10 text-xl font-700 text-center" max={maxPrice} />
        </div>
        ...
        <div className="flex gap-3">
          ...
          <button disabled={!price || +price > maxPrice || +price <= 0} className="btn-primary flex-1 disabled:opacity-40 disabled:cursor-not-allowed">שלח הצעה</button>
        </div>
      </div>
    </div>
  );
...
export default function EventPage() {
  ...
  useEffect(() => {
    async function load() {
      setLoading(true);
      const [ev, listings] = await Promise.all([
        getEvent(id),
  ...
  if (!event) {
    return <div className="text-center py-12 text-dark-400">האירוע לא נמצא</div>;
  }
  ...
  return (
    <div className="-mx-4 -mt-4">
      <div className="relative h-52 overflow-hidden">
        ...
        <button onClick={() => setSaved(!saved)} className="absolute top-4 left-4 p-2 rounded-full bg-black/30 backdrop-blur-sm">
      ...
      <div className="px-4 pt-4 pb-4">
        ...
        <div className="flex items-center gap-3 p-3 rounded-2xl bg-success-50 dark:bg-success-700/15 mb-4 trust-glow">
          ...
          <div>
            <p className="text-xs font-600 text-success-700 dark:text-success-300">עסקה מוגנת</p>
            <p className="text-[10px] text-success-600 dark:text-success-400">הכסף מוחזק אצלנו עד שהכרטיס אומת בכניסה לאירוע</p>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={() => setShowOffer(true)} className="btn-secondary text-xs py-1.5 px-3">הצע מחיר</button>
        </div>
